from keras.layers import Input, Dense, Conv1D, MaxPooling1D, UpSampling1D, Reshape, Flatten,BatchNormalization,GlobalMaxPooling2D
...
def cnn_1dae(encoder_dims,latent_dims= 256):
    ...
    x = Conv1D(32, 8, activation='relu',strides=1, padding='same')(reshaped_layer) 
    x = MaxPooling1D(pool_size=2)(x)
    #x=GlobalMaxPooling2D()(x)
    #x=BatchNormalization()(x)
    x = Conv1D(64, 7, activation='relu',strides=1, padding='same')(x) 
    x = MaxPooling1D(pool_size=2)(x) 
    x = Conv1D(128, 6, activation='relu',strides=1, padding='same')(x) 
    x = MaxPooling1D(pool_size=2)(x) 
    x = Conv1D(256, 5, activation='relu',strides=1, padding='same')(x)
    #x=BatchNormalization()(x)
    #x = MaxPooling1D(pool_size=2)(x) 
    ...
    x = Conv1D(128, 5, activation='relu', padding='same', name = 'decoder_7')(x) 
    x = UpSampling1D(2, name = 'decoder_6')(x) 
    x = Conv1D(64, 6, activation='relu', padding='same', name = 'decoder_5')(x) 
    x = UpSampling1D(2, name = 'decoder_4')(x) 
    x = Conv1D(32, 7, activation='relu', padding='same', name = 'decoder_3')(x) 
    x = UpSampling1D(2, name = 'decoder_2')(x) 
    x = Conv1D(1, 8, activation='linear', padding='same', name='decoder_1')(x) 
